...
def run_deduplication():
    ...
    try:
        ...
        for suffix in suffixes:
            ...
            count_before = duck_conn.execute(f"SELECT count(*) FROM {fact_table_name}").fetchone()[0]
            duck_conn.execute(dedup_query)
            count_after = duck_conn.execute(f"SELECT count(*) FROM {fact_table_name}").fetchone()[0]
