...
def dump_database_to_txt():
    ...
    try:
        # Targets the modern unified staging table
        query = """
            SELECT 
                forecast_date,
                average_wind_direction_deg,
                average_temperature_f,
                average_relative_humidity,
                upwind_compass_direction,
                downwind_compass_direction,
                processed_at AT TIME ZONE 'UTC' AT TIME ZONE 'America/New_York' AS processed_at
            ...
            if not result:
                ...
                for row in result:
                    ...
                    for header, value in zip(headers, row):
                        ...
                        # CLEANING STEP: If the database value contains raw text '\n' strings, strip them out
                        clean_value = str(value).replace("\\n", "").strip()
                        f.write(f"  {header}: {clean_value}\n")
                    f.write("\n")
                    
        print("Export complete! Open 'database_dump.txt' to view your clean records.")
