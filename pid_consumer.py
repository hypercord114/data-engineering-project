...
def get_dynamic_table_name():
    """Generates the table name based on the current date."""
    date_str = datetime.now(ZoneInfo("America/New_York")).strftime("%Y-%m-%d")
    return f"realtime_pid_telemetry_{date_str}"

def create_telemetry_table_if_not_exists(db_conn, table_name):
    ...
    with db_conn.cursor() as cur:
        query = f'CREATE TABLE IF NOT EXISTS "{table_name}" (' + """
                    ...
                    UNIQUE(sensor_id, reading_timestamp)
                );"""
        cur.execute(query)
        db_conn.commit()
...
def run_consumer():
    ...
    # Resolve daily table name dynamically
    current_table = get_dynamic_table_name()

    # 1. Initialize PostgreSQL Connection
    try:
        db_conn = psycopg2.connect(DB_CONNECTION_STRING)
        create_telemetry_table_if_not_exists(db_conn, current_table)
    ...
    try:
        while True:
            ...
            try:
                ...
                insert_telemetry_record(db_conn, reading_data, current_table)
