import logging  # <--- NEW: Import python logging engine
import math
import duckdb
import requests
...
# --- NEW LOGGING CONFIGURATION BLOCK ---
# This initializes a pipeline.log file and sets the row format structure
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(message)s",
    handlers=[
        logging.FileHandler("pipeline.log", encoding="utf-8"),  # Saves logs to file
        logging.StreamHandler(),  # Keeps logs visible in GitHub Actions terminal
    ],
)
...
def save_forecast_to_duckdb(
    ...
    try:
        ...
        logging.info(
            "Successfully recorded forecast data profile to local DuckDB file."
        )
...
def run_morning_etl():
    ...
    try:
        ...
        logging.info(
            f"Upstream data model verified. Last updated status: {meta_response.get('model_updates', {})}"
        )
        ...
        logging.error(f"API network request failed: {e}")
        return
    ...
    for time_str, wind, temp, humid in zip(
        hourly["time"],
    ...
    logging.info(
        f"Shift Summary for {today_date}: Temp: {avg_temp:.1f}F, Wind: {avg_wind:.1f} deg ({upwind}), Humidity: {avg_humid:.1f}%"
    )

    save_forecast_to_duckdb(
        today_date, avg_wind, avg_temp, avg_humid, upwind, downwind
    )
