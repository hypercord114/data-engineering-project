...
# Configured to append into the shared afternoon pipeline log file
logging.basicConfig(
    level=logging.INFO,
...
def create_table_if_not_exists(conn):
    """ Ensures a modern JSONB document repository schema exists in Postgres for observations. """
...
def load_to_postgresql(raw_json_string):
    ...
    try:
        ...
        # Insert the raw string directly into the observed table target
        query = """
            INSERT INTO daily_shift_weather_observed (forecast_date, raw_payload)
            VALUES (%s, %s)
        ...
        cur.close()
        conn.close()
        logging.info("Successfully committed complete observed JSONB document payload to cloud storage.")
...
def run_ingestion_pipeline():
    """ Orchestrates the end-to-end data acquisition workflow for afternoon observations. """
    today_date = datetime.now(ZoneInfo("America/New_York")).strftime("%Y-%m-%d")
    folder_name = "raw_json"
    
    # Ensure the directory exists in your GitHub workspace
    os.makedirs(folder_name, exist_ok=True)
    file_path = os.path.join(folder_name, f"observed_{today_date}.json")
    
    logging.info(f"--- Launching Afternoon Ingestion Phase for {today_date} ---")
    ...
    try:
        ...
        logging.error(f"PIPELINE INGESTION UNEXPECTED EXCEPTION: {e}")
        sys.exit(1)
